    ...
    public class NaniteMining
    {
        ...
        private bool CheckWorking()
        {
            ...
            foreach (var item in MyAPIGateway.Entities.GetEntitiesInSphere(ref sphere))
            {
                ...
                if (terminalBlock.BlockDefinition.SubtypeName == "NaniteUltrasonicHammer" || block.BlockDefinition.SubtypeName == "SmallNaniteUltrasonicHammer")
                {
        ...
        private void ScanVoxelWork()
        {
            try
            {
                if (m_oreList != null && DateTime.Now - m_lastRefresh < TimeSpan.FromMinutes(1))
                    return;

                m_lastRefresh = DateTime.Now;
                Vector3D position = m_block.GetPosition();
                BoundingSphereD sphere = new BoundingSphereD(position, 30f);
        ...
        */
        ...
        private bool IsInVoxel(IMyTerminalBlock block)
        {
            ...
            BoundingBoxD localAABB = new BoundingBoxD(cubeSize * ((Vector3D)block.Min - 5), cubeSize * ((Vector3D)block.Max + 5));
