...
namespace NaniteConstructionSystem.Entities
{
    public class NaniteConstructionBlock
    {
        ...
        {
            m_constructionBlock = entity as IMyTerminalBlock;
            if(m_constructionBlock == null)
            {
                Logging.Instance.WriteLine($"NaniteConstructionBlock.ctor: Entity is NULL");
                return;
            }

            var inventory = (m_constructionBlock as MyCubeBlock).GetInventory();
        ...
        private void ProcessInventory()
        {
            ...
            if(inventory.VolumeFillFactor > 0.50f && (GetTarget<NaniteDeconstructionTargets>().TargetList.Count > 0 || GetTarget<NaniteFloatingTargets>().TargetList.Count > 0 || GetTarget<NaniteMiningTargets>().TargetList.Count > 0))
            {
                GridHelper.FindFreeCargo((MyCubeBlock)m_constructionBlock, (MyCubeBlock)m_constructionBlock);
                Logging.Instance.WriteLine(string.Format("PUSHING Factory inventory over 50% full: {0}", m_constructionBlock.EntityId));
            }
        }
        ...
        public void SyncCompleteTarget(TargetData data)
        {
            try
            {
                ...
                {
                    var target = GetTarget<NaniteMiningTargets>().TargetList.FirstOrDefault(x => x as NaniteMiningItem != null && (x as NaniteMiningItem).Position == new Vector3D(data.PositionD.X, data.PositionD.Y, data.PositionD.Z)) as NaniteMiningItem;
                    if(target != null)
                    {
                ...
                block = GetTarget<NaniteConstructionTargets>().TargetList.FirstOrDefault(x => x as IMySlimBlock != null && (x as IMySlimBlock).Position == position && (x as IMySlimBlock).CubeGrid != null && (x as IMySlimBlock).CubeGrid.EntityId == data.TargetId) as IMySlimBlock;
                if (block != null)
                {
