...
namespace NaniteConstructionSystem.Entities.Effects
{
    public class MiningHammerEffect : NaniteBlockEffectBase
    {
        ...
        {
            m_block = block;
            isBig = (block.DefinitionDisplayNameText == "Nanite Ultrasonic Hammer Ore Locator - NUHOL");
            Logging.Instance.WriteLine("HammerType:" + block.DefinitionDisplayNameText+" isBig="+isBig);
        ...
        {
            if(isBig)MyCubeBlockEmissive.SetEmissiveParts(m_hammer, 0.0f, Color.FromNonPremultiplied(new Vector4(0.3f, 0.15f, 0.0f, 1f)), Color.White);
            m_tick++;
            ...
            else
            {
                ...
                if (m_tick % 90 == 0)
                {
                    ...
                    CreateParticles(Vector3D.Transform(new Vector3(0f, 0f, 0f), m_block.WorldMatrix), true, true, true);
        ...
        {
            if(isBig)MyCubeBlockEmissive.SetEmissiveParts(m_hammer, 1f, Color.Red, Color.White);
        ...
        {
            if(isBig)m_hammer.PositionComp.LocalMatrix = Matrix.CreateFromTransformScale(Quaternion.Identity, new Vector3(0f, 1f, 0f) + new Vector3(0f, 1.4f * m_position, 0f), Vector3.One);
        }

        public override void Unload()
        {
            StopParticles();
            if(isBig)m_hammer.Close();            
        }
    }
}
