...
namespace NaniteConstructionSystem.Entities.Effects
{
    public abstract class NaniteBlockEffectBase
    {
        ...
        private void GetModelsFolder()
        {
            ...
            {
                if (mod.PublishedFileId == 219757726 || mod.PublishedFileId == 1288970517)
            ...
            if (publishID != 0)
                ...
                m_modelsFolder = Path.GetFullPath(string.Format(@"{0}\{1}\Models\Cubes\large\", MyAPIGateway.Utilities.GamePaths.ModsPath, "CIDNaniteControlFactory"));
			Logging.Instance.WriteLine("m_modelsFolder="+m_modelsFolder);
		}
