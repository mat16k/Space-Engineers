...
namespace NaniteConstructionSystem
{
    ...
    public class NaniteConstructionManager : MySessionComponentBase
    {
        ...
        private static HashSet<long> m_assemblerBlocks;
        public static HashSet<long> AssemblerBlocks
        {
            get
            {
                if (m_assemblerBlocks == null)
                    m_assemblerBlocks = new HashSet<long>();
        ...
        private TerminalSettings m_terminalSettingsManager;
        ...
        public NaniteConstructionManager()
        {
            ...
            m_terminalSettingsManager = new TerminalSettings();
        }
        ...
        private void Initialize()
        {
            ...
            Logging.Instance.WriteLine("Initializing Sync");
            m_sync.Initialize();
            ...
            Logging.Instance.WriteLine("Initializing Controls");
            InitializeControls();

            Logging.Instance.WriteLine("Initializing Terminal Settings Manager");
            m_terminalSettingsManager.Load();

            if(Sync.IsClient)
            {
                Logging.Instance.WriteLine("Requesting Settings");

                m_sync.SendLogin();

                Logging.Instance.WriteLine("      Requesting Blocks");
                foreach (var item in NaniteBlocks)
                {
                    m_sync.SendNeedTerminalSettings(item.Key);
                }

                Logging.Instance.WriteLine("      Requesting Assembler Settings");
                foreach (var item in AssemblerBlocks)
                {
                    m_sync.SendNeedAssemblerSettings(item);
                }

                Logging.Instance.WriteLine("      Requesting Terminal Settings");
                foreach (var item in HammerTerminalSettings)
                {
                    m_sync.SendNeedHammerTerminalSettings(item.Key);
                }

                Logging.Instance.WriteLine("      Requesting Beacon Terminal Settings");
                foreach (var item in BeaconTerminalSettings)
                {
                    m_sync.SendNeedBeaconTerminalSettings(item.Key);
                }

                Logging.Instance.WriteLine("Completed Requests");
            }

            Logging.Instance.WriteLine("Initialization Completed");
        }
        ...
        private void CustomControlGetter(IMyTerminalBlock block, List<IMyTerminalControl> controls)
        {
            ...
            if(block.BlockDefinition.SubtypeName == "NaniteUltrasonicHammer" || block.BlockDefinition.SubtypeName == "SmallNaniteUltrasonicHammer")
            {
